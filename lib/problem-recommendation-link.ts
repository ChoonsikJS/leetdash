...
const BALANCED_PLATFORM_RECOMMENDATION_RULES = [
  "플랫폼별 추천 규칙:",
  "- LeetCode와 Programmers만 추천 대상으로 삼고, 두 플랫폼의 후보를 서로 독립적으로 선정한다.",
  "- 추천 기준에 맞는 문제가 충분하면 LeetCode 3개와 Programmers 3개를 추천한다.",
  "- 한 플랫폼의 추천 기준에 맞는 문제가 3개보다 적으면 확인된 문제까지만 추천하고, 해당 플랫폼에는 더 추천할 문제가 없다고 명시한다.",
  "- 부족한 수를 다른 플랫폼 문제나 추천 기준에 맞지 않는 문제로 억지로 채우지 않는다.",
  "- 각 추천의 플랫폼과 공식 문제 링크를 명확히 표시하고, 플랫폼별 추천 개수와 부족 여부를 요약한다.",
];
...
function buildNewUserProblemRecommendationPrompt(): string {
  return [
    ...
    "",
    ...BALANCED_PLATFORM_RECOMMENDATION_RULES,
    "",
    "다음 형식으로 한국어로 답하라:",
    "1. 시작 원칙: 이 순서로 학습하는 이유를 초보자 관점에서 2~3문장으로 설명.",
    "2. 문제 추천: 플랫폼별 추천 규칙에 따라 최대 6개를 선정하고, 순서, 플랫폼, 문제 이름, 난이도, 핵심 유형, 추천 이유, 문제 링크를 표로 제공.",
    "3. 플랫폼별 결과: LeetCode와 Programmers의 추천 개수 및 부족 여부를 요약.",
    "4. 첫 문제 안내: 가장 먼저 풀 문제 1개를 고르고, 문제를 읽을 때 확인할 점과 막혔을 때 볼 단계별 힌트 2개를 제공.",
    "5. 정답 코드나 완성된 풀이 코드는 제공하지 않는다.",
  ].join("\n");
}
...
export function buildProblemRecommendationPrompt(
  ...
  return [
    ...
    "4. 문제 추천: 공백을 먼저 보완하고 현재 풀이 난이도에서 한 단계씩 확장하도록 플랫폼별 추천 규칙에 따라 미풀이 문제를 최대 6개 추천. 각 문제의 플랫폼, 난이도, 보강 유형, 추천 이유, 문제 링크를 표로 제공.",
    "5. 플랫폼별 결과: LeetCode와 Programmers의 추천 개수 및 부족 여부를 요약.",
    "6. 요약: 지금 가장 먼저 풀 문제 1개와 그 이유를 한 문장으로 제시.",
  ].join("\n");
}
